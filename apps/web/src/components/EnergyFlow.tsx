...
export function EnergyFlow({ summary, charger, battery }: EnergyFlowProps): ReactElement {
  ...
  const nodes = [...new Set(links.flatMap((l) => [l.source, l.target]))].map((name) => ({
    name,
    itemStyle: { color: NODE_COLORS[name] ?? solar.series.grid, borderColor: 'transparent' },
  }));
    ...
    >
      ...
      {links.length <= 1 ? (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, py: 2 }}>
          ...
          <Box
            sx={{
              ...
              background: `linear-gradient(90deg, ${solar.series.production}, ${
                NODE_COLORS[links[0]?.target ?? 'House'] ?? solar.series.house
              })`,
              opacity: 0.85,
            }}
          />
          ...
          <Typography sx={{ font: `600 12px/1 ${solar.font.sans}`, color: NODE_COLORS[links[0]?.target ?? 'House'] ?? solar.series.house, flex: '0 0 auto' }}>
        ...
        <Box sx={{ display: 'flex', gap: 4, mt: 2, flexWrap: 'wrap' }}>
          {links.map((l) => (
            <Box key={`${l.source}-${l.target}`} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box sx={{ width: 10, height: 10, borderRadius: '3px', bgcolor: NODE_COLORS[l.target] ?? solar.series.grid }} />
