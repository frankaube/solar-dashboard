 ...
 */
export function ProductionCompare({ days }: { days?: number }): ReactElement {
  const [grouping, setGrouping] = useState<Grouping>('day');
  /*
    Keyed on both, so clicking Months or changing the page's range fetches now rather than
    in five minutes. The range used to reach one panel out of six on this page — and not
    this one, though it sits directly beneath the control.
  */
  const { data } = usePolling<ProductionBuckets>(
    () => fetchProductionBuckets(grouping, days),
    POLL_MS,
    `${grouping}:${days ?? ''}`,
  );
