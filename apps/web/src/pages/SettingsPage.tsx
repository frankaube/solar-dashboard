 ...
 *
 * `help` is a hint rather than a caption. Every field here carried a sentence or two
 * permanently, and on the Rates tab — eight rows — that was most of about 380 words,
 * explaining fields whose labels already said what they were. Nothing was cut; it moved
 * behind the ⓘ, still one gesture away and still read aloud by a screen reader.
 *
 * `urgent` is the exception. A couple of these are not explanation but consequence — the
 * kind of thing that is useless after the fact — and those stay on the page.
 */
function Row({
  label,
  help,
  ...
  label: string;
  help?: string;
  /** Keep the text on the page rather than behind the hint. */
  urgent?: boolean;
  children: ReactNode;
}): ReactElement {
  return (
    ...
    >
      <Box>
        <Typography variant="body2" component="div" sx={{ color: solar.ink.pri }}>
          {label}
          {help && !urgent && <Hint>{help}</Hint>}
        </Typography>
        {help && urgent && (
      ...
      */}
      ...
      <Panel active={current.id === 'rates'}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          {/*
            Directly under the rates it qualifies: the price above is what applies now, and
            this is what applied before.
          */}
          <RateHistoryCard />
          <UtilityUsageCard />
        </Box>
      ...
      <Panel active={current.id === 'hardware'}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {/*
            First, because more of the app depends on it than on anything else in this tab,
            and because until recently there was nowhere at all to set it.
          */}
          <SiteLocationCard />
          {/* Directly under the location it depends on, and cannot be switched on without. */}
          <RadarCard />
          <HardwareCard />
          {/*
            How to reach this install, beside what it talks to. It is a fact about this
            machine rather than about the array, which is what the rest of this tab holds.
          */}
          <LocalNameCard />
          {/*
      ...
      <Panel active={current.id === 'data'}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          <DataCard />
          {/*
            Beside the export tools, because it is the same subject from the other side:
            what this machine holds, and what to do when it is missing a piece.
          */}
          <GapFillCard />
          {/*
