...
function RatesCard(): ReactElement {
  ...
  const server: Record<RateKey | "programId" | "includesTax", string> | null = config
    ? {
    ...
    : null;
  const { values, dirty, set, clear } = useEditable<RateKey | "programId" | "includesTax">(server);
  const caps = useCapabilities();
      ...
      <Box sx={{ display: 'flex', gap: 3, alignItems: 'center', mt: 4 }}>
        ...
        */}
        <Button variant="contained" onClick={() => void save()} disabled={!dirty || saving}>
          {saving ? <CircularProgress size={18} sx={{ color: '#0e0d0b' }} /> : 'Save changes'}
        </Button>
        {status && (
          <Typography
            variant="caption"
    ...
    <Surface title="Hardware">
      ...
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', mt: 4 }}>
        ...
        <Button
          ...
          sx={{ mt: 0.5 }}
        >
          {scanning ? <CircularProgress size={18} sx={{ color: '#0e0d0b' }} /> : 'Scan network'}
        </Button>
      </Box>
