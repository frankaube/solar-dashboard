...
/** Read per render, not at import — see EnergyFlow for what a frozen capture costs. */
const foregone = (): string => solar.series.expected;
...
/**
 * Where a period's self-consumption figure came from.
 *
 * Three states, not two, because the honest answer is usually the middle one. A published
 * usage export lags by weeks, so "most of this month was measured at the meter and the
 * last few days are your estimate" is the normal condition rather than an edge case — and
 * a bare asterisk labels that identically to a period nobody measured at all.
 */
export function selfConsumptionBasis(p: SavingsPeriod, anyMeasurement = false): string {
  const measured = p.measuredProducedKwh ?? 0;
  const share = p.producedKwh > 0 ? measured / p.producedKwh : 0;
  const kwh = (n: number): string => `${Math.round(n).toLocaleString('en-CA')} kWh`;

  if (!p.selfConsumptionEstimated && measured > 0) {
    return `Measured at the meter: what the array made, minus what actually left the property, across all ${kwh(p.producedKwh)}.`;
  }
  if (share > 0.01) {
    return (
      `Measured at the meter for ${kwh(measured)} of ${kwh(p.producedKwh)} (${Math.round(share * 100)}%). ` +
      'The rest uses the share estimated in Settings — your utility publishes in arrears, so the most recent days are always the estimated ones.'
    );
  }
  /*
    Nothing measured in THIS period, which on an install that imports regularly is simply
    today and yesterday — the days the utility has not published yet. Telling that owner to
    import their usage export is advice they have already taken, and the fastest way to
    teach someone that the explanations on a page are not worth reading.
  */
  if (anyMeasurement) {
    return 'No meter reading covers these days yet — your utility publishes in arrears. The share estimated in Settings applies until it does.';
  }
  return 'Based on the share you estimated in Settings, not a meter. Import your utility usage export, or clamp a meter on the service entrance, to measure it instead.';
}
      ...
      */}
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 5 }}>
        ...
        <Surface title={`Use it or export it — the ${hstPct}% rule`}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {[
              ...
              { label: 'Sent to the grid', v: exportRate, color: solar.ink.faint, ink: solar.ink.sec },
            ].map((r) => (
              <Box key={r.label} sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                <Typography sx={{ width: 128, flex: '0 0 128px', font: `500 13px/1.3 ${solar.font.sans}`, color: r.color === solar.ink.faint ? solar.ink.dim : solar.series.production }}>
                  {r.label}
                </Typography>
        ...
        <Surface title="By period">
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            {rows.map(({ label, p }) => (
              <Box key={label} sx={{ display: 'flex', alignItems: 'center', py: '13px', borderBottom: '1px solid', borderColor: solar.grid.line }}>
                ...
                */}
                <Tooltip title={selfConsumptionBasis(p, (lifetime.measuredProducedKwh ?? 0) > 0)}>
                  <Typography variant="mono" sx={{ width: 88, textAlign: 'right', pl: 2, fontSize: 13, color: solar.ink.dim, cursor: 'help' }}>
          ...
          {/*
            Days the meter recorded no export used to be stated here as a standing fact.

            Wrong shape for it. Days that ended months ago are history — priced on the
            credit card, where the kWh nobody was paid for is the point — while days still
            running are money leaving this week, and a line that reads identically for both
            carries no signal. The live case is an alert now; see `unmetered-export.ts`.
          */}
          <Link
            component="button"
            type="button"
