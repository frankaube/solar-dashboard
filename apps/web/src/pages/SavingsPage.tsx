...
export function SavingsPage(): ReactElement {
  ...
  // Only for the unmetered caveat below; null on an install that has imported nothing.
  const { data: utility } = usePolling<UtilityUsageStatus>(fetchUtilityUsage, POLL_MS);
      ...
      */}
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 5 }}>
        ...
        <Surface title="By period">
          ...
          {/*
            Days the meter recorded no export while the array was producing.

            Surfaced here rather than only at import time, because this is the page whose
            numbers it changes and the import screen is visited once. Left as a fact with a
            figure attached rather than an instruction: what to do about it is between the
            owner and their utility, and the app's job is to make sure it is not invisible.
          */}
          {utility && utility.unmeteredDays > 0 && (
            <Typography
              variant="caption"
              sx={{ display: 'block', mt: 2, color: solar.status.warn, maxWidth: 620 }}
            >
              {utility.unmeteredDays} imported day(s) show production against no export at
              all — a meter that was not counting what left the property, usually
              net metering not yet activated. Those days are excluded from the measured
              figures above and fall back to your estimate.
            </Typography>
          )}
          <Link
            component="button"
