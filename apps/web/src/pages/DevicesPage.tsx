...
const TONE: Record<StateTone, { bg: string; border: string; ink: string }> = {
  ok: { bg: '#1c2b21', border: '#2f4a37', ink: solar.status.ok },
  warn: { bg: '#332417', border: '#6b4a24', ink: solar.status.warn },
  bad: { bg: '#331d1c', border: '#6b2f2c', ink: solar.status.critical },
  idle: { bg: '#24211d', border: '#35302a', ink: solar.ink.dim },
};
              ...
              const width = u.onHoursPerDay > 0 ? Math.round((u.onHoursPerDay / busiest) * 100) : 0;
                ...
                return (
                  <Box key={u.deviceId} sx={{ display: 'flex', flexDirection: 'column', gap: 0.9 }}>
                    ...
                    <Box sx={{ height: 8, borderRadius: '3px', background: '#24211d', overflow: 'hidden' }}>
                      <Box
                        sx={{
                          width: `${width}%`,
                          height: '100%',
