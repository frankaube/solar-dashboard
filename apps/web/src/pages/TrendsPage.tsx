...
export function TrendsPage(): ReactElement {
  ...
  const { data: energy } = usePolling(() => fetchEnergyHistory(days), SLOW_POLL_MS);
  ...
  const { data: charging } = usePolling(() => fetchChargeSessions(days), SLOW_POLL_MS);
  ...
  const energyOption = {
    ...preset,
    xAxis: { ...(preset.xAxis as object), type: 'category' as const, data: (energy ?? []).map((d) => d.date.slice(5)) },
    tooltip: { ...(preset.tooltip as object), trigger: 'axis' as const },
    series: [
      {
        type: 'bar' as const,
        data: (energy ?? []).map((d) => Number((d.energyWh / 1000).toFixed(2))),
        barWidth: '58%',
        itemStyle: {
          color: (p: { dataIndex: number }) =>
            p.dataIndex === (energy?.length ?? 0) - 1 ? solar.series.production : '#8a6a2e',
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      <Surface
        title={
          <Box>
            <Typography variant="subtitle1">Daily energy · kWh</Typography>
            <Typography variant="caption" color="text.disabled">
              today highlighted
            </Typography>
          </Box>
      ...
      >
        <Chart option={energyOption} height={180} />
      </Surface>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {charging.sessions.slice(0, 8).map((session) => (
                ...
                >
                  ...
                  <Box sx={{ flex: 1, minWidth: 80, height: 6, borderRadius: '3px', bgcolor: '#2e2a25', overflow: 'hidden' }}>
                    <Box
                      sx={{
