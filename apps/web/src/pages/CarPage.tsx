...
export function CarPage(): ReactElement {
  ...
  const carAnswer = charging
    ? `Charging${chargeSourcePhrase} — ${(chargeW / 1000).toFixed(1)} kW going in.`
    : plugged
      ? 'Plugged in at home.'
      : `Parked in the garage${vehicle.batteryLevel !== null ? ` — ${vehicle.batteryLevel}% charged` : ''}.`;
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
            <Box
              ...
              sx={{
                ...
                bgcolor: charging ? '#251f33' : '#24211d', border: `1px solid ${charging ? '#4a3d68' : '#3a352e'}`,
                color: charging ? '#b3a3e0' : solar.ink.dim, font: `600 12px/1 ${solar.font.sans}`, flex: '0 0 auto',
              }}
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth={1.8}>
                <path d="M6.6 1.6L2.8 6.8h2.6l-.6 3.6 3.8-5.2H6l.6-3.6z" />
              </svg>
              {charging ? 'Charging' : plugged ? 'Plugged in' : (vehicle.state ?? 'Idle')}
            </Box>
