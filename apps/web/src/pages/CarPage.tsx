...
export function CarPage(): ReactElement {
  ...
  /*
    The gasoline comparison is computed on the server now, drive by drive, each at the
    published fuel price for the month it happened in. It used to be two constants in this
    file — 9 L/100 km and $1.60/L — multiplied by the whole period's distance. Over
    eighteen months the published average ran 130.0¢ to 191.1¢, so a flat current price
    did not misprice an old drive slightly; it misprices it by half. Constants in a
    rendering component are also unreachable: not merely wrong, but unfixable by the
    person looking at them.
  */
  const gas = details?.gasComparison ?? null;
  const gasSavings = gas && stats ? gas.gasCost - stats.energyUsedKwh * pricePerKwh : null;
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 460px' }, gap: 5 }}>
        ...
        <Surface title="Instead of gasoline">
          ...
          <Typography variant="answer" sx={{ fontSize: 14, lineHeight: 1.55, color: solar.ink.sec, display: 'block', mt: 3 }}>
            {gas && stats
              ? `${stats.drivenKm.toLocaleString()} km driven — a gas car would've burned about ${Math.round(gas.litres).toLocaleString()} L.`
              : stats
                ? 'Set a comparison car and a place in Settings, and this fills in from published fuel prices.'
                : `Trip data appears as ${caps?.vehicle?.name ?? 'your vehicle logger'} logs drives.`}
          </Typography>
          {/*
            Every assumption behind the number, in the order they matter.

            `basis` comes from the server because it is generated from the same arithmetic
            that produced the figure — a caveat maintained separately drifts out of step
            and then reads as diligence while being wrong. The litres-per-100km stays an
            assumption however good the prices get: it describes a car nobody bought.
          */}
          {gas && (
            <Typography variant="caption" color="text.disabled" sx={{ mt: 2, display: 'block' }}>
              Against a {gas.litresPer100Km} L/100 km petrol car, minus the{' '}
              {((config?.electricityPricePerKwh ?? FALLBACK_PRICE_PER_KWH) * 100).toFixed(1)}¢/kWh the
              charging cost. {gas.basis}
            </Typography>
          )}
          <Box sx={{ height: '1px', bgcolor: solar.surface.border, my: 4 }} />
          <Link
