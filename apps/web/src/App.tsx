...
import { useThemeMode } from './shell/ThemeMode';
...
export function App(): ReactElement {
  /*
    Subscribing here is what makes a theme change reach the charts.

    ThemeModeProvider sits above this in main.tsx and receives `children` as an element
    built during the root render, which never re-runs — so React saw the same element and
    bailed out of the whole subtree. MUI's own styling still updated, because that flows
    through context to styled components, and the result was a page whose surfaces went
    light while every canvas kept painting the dark palette's gold. Reading the mode here
    re-renders the tree, and the chart options are rebuilt each render.

    Deliberately at the top and above the early returns below: called unconditionally, so
    the hook order cannot change.
  */
  useThemeMode();
