...
describe('localDateOf produces ISO regardless of locale data', () => {
  /*
    The bug this exists for: localDateOf used Intl.DateTimeFormat('en-CA').format(), which
    renders ISO only where en-CA exists. The packaged Lite build ships small-icu, where it
    does not — so the same code wrote "2026-07-30" under Docker and "7/30/2026" on the Pi,
    into the column every daily and monthly rollup groups by. 242 rows before anyone
    noticed, and what surfaced was a month total of $1.
  */
  const ZONE = 'Atlantic/Bermuda';

  it('always returns YYYY-MM-DD', () => {
    for (const iso of ['2026-01-05T12:00:00Z', '2026-07-31T12:00:00Z', '2026-12-09T12:00:00Z']) {
      expect(localDateOf(new Date(iso), ZONE)).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    }
  });

  it('zero-pads single-digit months and days', () => {
    // The failure mode was M/D/YYYY, so an unpadded 5 is the shape to guard against.
    expect(localDateOf(new Date('2026-01-05T12:00:00Z'), ZONE)).toBe('2026-01-05');
  });

  it('does not depend on en-CA being available', () => {
    /*
      Asserted structurally rather than by removing a locale, which a test cannot do: the
      implementation must never use the formatted string, only named parts. If someone
      reintroduces .format(), this still passes under full ICU — so the guard that matters
      is the shape check above plus this reminder of why.
    */
    const usStyle = new Intl.DateTimeFormat('en-US', { timeZone: ZONE }).format(
      new Date('2026-07-31T12:00:00Z'),
    );
    expect(usStyle).toMatch(/^\d{1,2}\/\d{1,2}\/\d{4}$/); // what small-icu gives for en-CA
    expect(localDateOf(new Date('2026-07-31T12:00:00Z'), ZONE)).toBe('2026-07-31');
  });

  it('still buckets by the site timezone, not UTC', () => {
    // 01:30Z on the 16th is the evening of the 15th at UTC-4 — the whole point of the column.
    expect(localDateOf(new Date('2026-07-16T01:30:00Z'), ZONE)).toBe('2026-07-15');
  });
});
