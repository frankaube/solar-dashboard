...
/** UTC-4 going to UTC-3 in summer — chosen for the offset, not the place. */
...
describe('backup filenames', () => {
  ...
  it('uses local time, so the name matches the day its owner would call it', () => {
    /*
      ...
      The zone is passed explicitly. Without it this asserted "whatever timezone this
      computer is set to" — which is Atlantic here and UTC on a CI runner, so it passed
      on the machine that wrote it and failed the first time anything else ran it.
    */
    expect(stamp(new Date('2026-07-16T01:30:00Z'), ZONE)).toMatch(/^20260715-/);
  });
  ...
  it('never emits hour 24', () => {
    ...
    expect(stamp(new Date('2026-07-15T03:00:00Z'), ZONE)).toMatch(/-00\d\d$/);
  });
});
