...
interface Opts {
  ...
  monthWh?: number;
  /*
    Settable independently, because a period's TOTAL comes from the stats rollup while its
    RATE comes from the daily rows filtered by the same window. In the real service those
    agree — both derive from the same readings — so a fixture where they disagree asks a
    question the app cannot actually be asked.
  */
  yearWh?: number;
  lifetimeWh: number;
  retail: number;
  ...
  /** Whether to derive that share from the meter instead of using the typed one. */
  ...
  /** Recorded rate changes, when a case is about historical pricing. */
  rates?: Array<{
...
function makeService(opts: Opts): SavingsService {
  const readings = {
    getEnergyStats: async () => ({
      ...
      yearWh: opts.yearWh ?? opts.lifetimeWh,
    ...
    getConfig: async () => ({
      ...
      selfConsumptionAuto: opts.selfConsumptionAuto ?? false,
    }),
    /*
  ...
  const prisma = {
    ...
    */
    ...
    /*
      No recorded rate changes unless a case asks for them. Every existing test therefore
      exercises the fallback — the single configured price — which is the state of every
      install that has never entered one, and the reason this change moves nobody's numbers.
    */
    rateEntry: {
      findMany: async () => opts.rates ?? [],
    },
    deviceReading: {
      findMany: async () =>
...
describe('SavingsService with a mains clamp', () => {
  ...
  it('will not let a clamp fitted today rewrite the years before it', async () => {
    ...
    */
    ...
    expect(s.today.selfConsumptionEstimated).toBe(false);
    /*
      Lifetime is measured where the clamp reaches and estimated everywhere else — 28 kWh
      from today's readings plus 30% of the 870 kWh nobody watched. What it must never do
      is subtract a new clamp's exports from the whole total and report 890 of 900 kWh as
      used at home, which is the failure the per-day split exists to prevent.
    */
    expect(s.lifetime.selfConsumptionEstimated).toBe(true);
    expect(s.lifetime.selfConsumedKwh).toBeCloseTo(28 + 870 * 0.3, 0);
    expect(s.lifetime.selfConsumedKwh).toBeLessThan(400);
    // And it says how much of the period the measurement actually covered.
    expect(s.lifetime.measuredProducedKwh).toBeCloseTo(30, 0);
    expect(s.today.measuredProducedKwh).toBeCloseTo(30, 0);
  });

  it('measures the days it can and estimates the rest, rather than discarding both', async () => {
    /*
      The flaw the per-day split fixes. A published usage export always lags — the
      utility's file ends weeks before today — so demanding that a meter cover the WHOLE
      period meant no period was ever covered, and every imported measurement was thrown
      away the moment it arrived. Seen on a real install: four periods, all still reporting
      the owner's estimate, minutes after importing a month of real meter data.
    */
    const s = await makeService({
      todayWh: 30_000,
      monthWh: 130_000,
      ...
      hst: 0.15,
      selfConsumptionPct: 30,
      utilityExport: { '2026-08-01': 40, '2026-08-02': 30 },
      daily: [
        ['2026-08-01', 60_000], // measured: 60 produced, 40 exported → 20 stayed home
        ['2026-08-02', 40_000], // measured: 40 produced, 30 exported → 10 stayed home
        [today(), 30_000], // no meter reading yet → the owner's 30% applies to this alone
      ],
    }).getSavings();

    // 30 kWh measured across the two metered days, plus 30% of the 30 kWh not yet published.
    expect(s.month.selfConsumedKwh).toBeCloseTo(30 + 30 * 0.3, 0);
    expect(s.month.measuredProducedKwh).toBeCloseTo(100, 0);
    // Two thirds of a period measured is worth far more than the flat estimate it replaces.
    expect(s.month.selfConsumedKwh).toBeLessThan(130 * 0.3 + 10);
    // Still flagged: part of the period rests on the assumption, and a flag that only
    // fired at fully-assumed would call a three-quarters-measured period a measurement.
    expect(s.month.selfConsumptionEstimated).toBe(true);
  });

  /*
    A fortnight of metered days where half of everything produced stayed home, plus one
    day nobody metered. The measured share is 50%; the owner typed 10%. Which of the two
    gets applied to that last day is the entire question the toggle answers.
  */
  const CALIBRATION = {
    daily: [
      ...Array.from({ length: 14 }, (_, i) => [`2026-07-${String(i + 1).padStart(2, '0')}`, 50_000] as [string, number]),
      [today(), 30_000] as [string, number],
    ],
    utilityExport: Object.fromEntries(
      Array.from({ length: 14 }, (_, i) => [`2026-07-${String(i + 1).padStart(2, '0')}`, 25]),
    ),
    todayWh: 30_000,
    lifetimeWh: 730_000,
    retail: 0.16,
    hst: 0.15,
    selfConsumptionPct: 10,
  };

  it('applies the typed share to unmetered days while the toggle is off', async () => {
    const s = await makeService(CALIBRATION).getSavings();
    // 350 kWh measured across the fortnight, plus the owner's 10% of the unmetered 30.
    expect(s.lifetime.selfConsumedKwh).toBeCloseTo(353, 0);
  });

  it('applies the share its own meter measured once asked to', async () => {
    /*
      The same 350 kWh measured, but the 30 kWh nobody watched is now valued at the 50%
      this house actually ran at rather than the 10% typed into a box. Still an assumption
      on that day — just one calibrated on this roof instead of guessed.
    */
    const s = await makeService({ ...CALIBRATION, selfConsumptionAuto: true }).getSavings();
    expect(s.lifetime.selfConsumedKwh).toBeCloseTo(365, 0);
    // And it stays flagged: a derived share is still an estimate where no meter reached.
    expect(s.lifetime.selfConsumptionEstimated).toBe(true);
  });

  it('keeps the typed share when there is too little meter data to derive one', async () => {
    /*
      Switching this on before the meter has much to say must not quietly zero the
      assumption — which is what taking a null share as 0% would do, valuing every
      unmetered kWh as exported and understating the savings of anyone who opted in early.
    */
    const s = await makeService({
      ...CALIBRATION,
      selfConsumptionAuto: true,
      daily: [CALIBRATION.daily[0], [today(), 30_000]],
      utilityExport: { '2026-07-01': 25 },
      lifetimeWh: 80_000,
    }).getSavings();
    expect(s.lifetime.selfConsumedKwh).toBeCloseTo(25 + 30 * 0.1, 0);
  });
...
describe('a price that changed', () => {
  /*
    The whole point. One price applied to all history means every figure changes
    retroactively the day a utility raises its rate — a savings total written down last
    winter stops matching the one on screen, and nothing says why.
  */
  const RATES = [
    { effectiveFrom: '2025-01-01', pricePerKwh: 0.1, hstRate: 0, priceIncludesTax: true },
    { effectiveFrom: '2026-01-01', pricePerKwh: 0.2, hstRate: 0, priceIncludesTax: true },
  ];

  it('values the old days at the old price and the new ones at the new', async () => {
    /*
      100 kWh at 10c and 100 kWh at 20c is $30 — not $20 as one flat old rate would say,
      and not $40 as today's rate applied backwards would.
    */
    const s = await makeService({
      todayWh: 0,
      monthWh: 0,
    ...
    expect(s.lifetime.grossValue).toBeCloseTo(30, 1);
  });

  it('weights by production, not by how long each rate was in effect', async () => {
    // Nine tenths of the energy came at the higher rate, so the year is worth far more
    // than the midpoint of the two prices.
    const s = await makeService({
      todayWh: 0,
      monthWh: 0,
      lifetimeWh: 100_000,
    ...
    // 10 kWh at 10c plus 90 kWh at 20c = $19, against $15 for an unweighted average.
    expect(s.lifetime.grossValue).toBeCloseTo(19, 1);
  });

  it('prices a period by its own days, so this year differs from lifetime', async () => {
    const s = await makeService({
      todayWh: 0,
      monthWh: 0,
      yearWh: 100_000,
    ...
    // The year window sees only 2026 production, so all of it is at 20c: $20.
    expect(s.year.grossValue).toBeCloseTo(20, 1);
    // Lifetime spans both rates, so its per-kWh average is lower — but on twice the energy.
    expect(s.lifetime.grossValue).toBeCloseTo(30, 1);
  });

  it('falls back to the configured price when nothing has been recorded', async () => {
    // Every existing install. The change must be invisible until somebody uses it.
    const s = await makeService({
      todayWh: 0,
      monthWh: 0,
      lifetimeWh: 100_000,
      retail: 0.16,
      hst: 0,
      daily: [['2025-06-01', 100_000]],
    }).getSavings();
    expect(s.lifetime.grossValue).toBeCloseTo(16, 1);
  });

  it('quotes today s rate for what one more kWh is worth', async () => {
    /*
      Two different questions on one page. The periods answer "what was it worth", which is
      historical; `marginal` answers "what is it worth now", which is not. Publishing the
      configured price alongside history priced from the recorded rates would show two
      answers, and the one people act on would be the stale one.
    */
    const s = await makeService({
      todayWh: 0,
      monthWh: 0,
      lifetimeWh: 200_000,
      retail: 0.99, // the old configured price, deliberately absurd
      hst: 0,
      rates: [
        { effectiveFrom: '2025-01-01', pricePerKwh: 0.1, hstRate: 0, priceIncludesTax: true },
        { effectiveFrom: '2026-01-01', pricePerKwh: 0.2, hstRate: 0, priceIncludesTax: true },
    ...
    // The clock is pinned to 2026-08-04, so the rate in effect is the 2026 one.
    expect(s.rates.retailPerKwh).toBeCloseTo(0.2, 6);
    // And not the configured price, which no longer governs anything once history exists.
    expect(s.rates.retailPerKwh).not.toBeCloseTo(0.99, 2);
    // While the periods still carry their own blend.
    expect(s.lifetime.grossValue).toBeCloseTo(30, 1);
  });
});
