...
import { notifyReady } from './common/watchdog';
...
async function bootstrap(): Promise<void> {
  ...
  */
  ...
  /*
    The gap-fill import takes either a pasted block or a saved file, so JSON keeps going to
    the JSON parser and everything else arrives as bytes. Without this a text/plain upload
    reaches the controller as an empty object and reports "nothing to import" about a file
    that plainly had rows in it.
  */
  app.use(
    '/api/readings/cloud-import',
    raw({
  ...
  /*
    Told after the port is open, not before. systemd holds dependent units until this
    arrives, and the watchdog only starts counting once a service is ready — announcing it
    early would start the clock while the app was still opening its database.
  */
  notifyReady();
