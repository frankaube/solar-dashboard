...
export function isDue({
  ...
  // Never backed up: do it now rather than waiting for the first anchor to come round.
  if (!lastSuccess) return true;

  const elapsed = now.getTime() - lastSuccess.getTime();
  if (!spec.anchored) return elapsed >= spec.ms;
  if (elapsed < spec.ms - ANCHOR_GRACE_MS) return false;
  return ranBeforeSlot(lastSuccess, now, normaliseHour(hour), timeZone);
}
