...
export class BackupService implements OnModuleInit, OnModuleDestroy {
  ...
  async runNow(
    only?: string[],
    /**
     * True only when the schedule asked. Ad-hoc runs — the button, a deploy — record a
     * success but must not move the anchor, or the nightly slot drifts by a day every
    ...
    try {
      ...
      for (const { kindId, dest } of targets) {
        ...
        try {
          ...
          // Only a scheduled run moves the anchor. A deploy or a button press must not
          // push the nightly slot out by a day — see isDue().
          if (scheduled) await this.set(stateKey(kindId, 'lastScheduledAt'), at.toISOString());
  ...
  /** When the SCHEDULE last ran, as opposed to any backup at all. */
  private async lastScheduledFor(kindId: string): Promise<Date | null> {
    const raw = await this.setting(stateKey(kindId, 'lastScheduledAt'));
    return raw ? new Date(raw) : null;
  }
  ...
  private async runIfDue(): Promise<void> {
    ...
    for (const { kindId } of await this.activeDestinations()) {
      ...
      const lastScheduled = await this.lastScheduledFor(kindId);
      if (isDue({ now, lastSuccess, lastScheduled, frequency, hour })) due.push(kindId);
    }
    if (due.length) await this.runNow(due, true);
  }
 ...
 *
 * The zone is a parameter, defaulting to the site's, for the same reason `localDateOf`
 * takes one: SITE_TIMEZONE resolves once at import from the environment and falls back to
 * the machine's own zone. A test asserting UTC-4 behaviour without passing a zone is
 * really asserting "whatever this computer is set to" — which passed on a workstation in
 * Atlantic Canada and failed the moment CI ran it in UTC.
 */
export function stamp(at: Date, timeZone: string = SITE_TIMEZONE): string {
