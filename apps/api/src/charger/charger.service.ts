...
import { solarShareOf } from './solar-overlap';
...
export class ChargerService implements OnModuleInit, OnModuleDestroy {
  ...
  async getSessions(days: number): Promise<{
    ...
    const finalize = (): void => {
      ...
      /*
        The same overlap the car-derived figures use — one implementation, not two.

        This method had its own copy of the integral and its own SOLAR_PAIRING_WINDOW_MS
        and MAX_SAMPLE_GAP_MS, identical to the module's. Two live copies meant the same
        charge could be given different percentages depending on which endpoint rendered
        it, and a correction to one would silently miss the other.
      */
      const share = solarShareOf(current, solar, energyWh);
      sessions.push({
        ...
        solarWh: share.solarWh,
        solarPct: share.solarPct,
