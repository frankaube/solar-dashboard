...
/**
 * How long to wait for a TCP connect during a sweep.
 *
 * This only ever elapses on addresses with nothing there — a device on the same LAN
 * answers in single-digit milliseconds, and a host that is present but has the port shut
 * refuses immediately. So it is purely "how long to spend being sure an address is empty",
 * and at 1,200 ms it was most of a three-minute scan.
 *
 * 700 ms is still a hundred times a LAN round trip, which leaves room for an embedded
 * device that is slow to accept while cutting the wait on the 250 addresses where nothing
 * is listening.
 */
...
export class DevicesService implements OnModuleInit, OnModuleDestroy {
   ...
   */
  /**
   * Mark a meter as the one clamped on the service entrance, or clear the mark.
   *
   * Exclusive, and enforced here rather than left to the caller: two devices claiming to
   * measure the whole property would have the savings engine pick whichever the database
   * returned first, silently, and a house with a sub-panel meter adopted later is exactly
   * how that happens. The previous holder is demoted in the same transaction, so there is
   * no instant where both or neither is set.
   *
   * Only a meter can hold it. A smart plug reports real watts and would integrate to a
   * perfectly plausible export figure for one appliance, which is the worst kind of wrong:
   * every downstream number stays believable.
   */
  async setRole(id: number, role: 'mains' | null): Promise<Device> {
    const device = await this.prisma.device.findUnique({ where: { id } });
    if (!device) throw new NotFoundException('unknown device');
    if (role === 'mains' && device.kind !== 'meter') {
      throw new BadRequestException(
        `only a meter can be the mains — "${device.name}" is a ${device.kind}, and one appliance's power is not the property's`,
      );
    }
    const [, updated] = await this.prisma.$transaction([
      this.prisma.device.updateMany({
        where: { role: 'mains', id: { not: id } },
        data: { role: null },
      }),
      this.prisma.device.update({ where: { id }, data: { role } }),
    ]);
    return updated;
  }
