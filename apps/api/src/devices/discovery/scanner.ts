 ...
 *
 * This was 8, which made a scan take just under three minutes on a Pi and read as broken.
 * The arithmetic is unforgiving: 254 hosts at 8 a time is 32 batches, each waiting out the
 * connect timeout on every silent address, once per distinct port — four ports here, so
 * 32 × 1.32 s × 4 ≈ 169 s. Measured: 169.7 s.
 *
 * The modesty was justified by embedded HTTP stacks wedging under bursts, which is real —
 * but that is a burst against ONE device. These are single connects to 32 DIFFERENT
 * addresses; no individual device sees more than one. The two were conflated.
 */
const SWEEP_CONCURRENCY = 32;
/**
 * Gap between batches. Still here, and still deliberate: it is what keeps a sweep from
 * looking like a SYN flood to a router's connection tracker. At 8 batches a port it costs
 * under a second.
 */
const PACING_MS = 120;
