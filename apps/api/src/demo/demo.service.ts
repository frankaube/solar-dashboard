...
export class DemoHouse {
  ...
  vehicleDetails(): object {
    ...
    const drives = Array.from({ length: 12 }, (_, i) => ({
      ...
      from: 'Home',
      to: ['Work', 'Grocery', 'Shediac', 'Gym'][i % 4],
    ...
    return {
      details: {
        ...
        battery,
        drives,
        charges: (this.chargerSessions(90) as { sessions: object[] }).sessions.slice(0, 10).map((s) => ({
          startedAt: (s as { startedAt: string }).startedAt,
          location: 'Home',
          energyAddedKwh: (s as { energyWh: number }).energyWh / 1000,
          energyUsedKwh: (s as { energyWh: number }).energyWh / 1000 / 0.9,
          durationMin: 240,
          startLevel: 40,
          endLevel: 80,
          fast: false,
