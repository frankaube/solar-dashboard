...
export class UtilityImportService {
  ...
  /**
   * Every imported day, both directions, oldest first.
   *
   * Unlike `exportedKwhByDate` this keeps the unmetered days and says so, because the
   * credit bank needs them: a day the meter counted import but not export still drew the
   * balance down, and dropping it would report a bank richer than the utility's.
   */
  async meterDays(): Promise<
    Array<{ date: string; importedKwh: number; exportedKwh: number; unmetered: boolean }>
  > {
    return this.prisma.utilityReading.findMany({
      orderBy: { date: 'asc' },
      select: { date: true, importedKwh: true, exportedKwh: true, unmetered: true },
    });
  }
