...
export class SavingsService {
  ...
  async getSavings(): Promise<SavingsDto> {
    ...
    */
    ...
    const [evByDate, battByDate] = await Promise.all([
    ...
    /*
      ...
      Only solar diverted to an EV or a battery is measurable here — the fridge, the heat
      pumps and the water heater are invisible without a whole-home meter, so measured
      self-consumption reads far too low and nearly every kWh gets valued at the export
      rate. Where the owner has told us what share their house really uses directly, take
      whichever is larger: the measurement is a hard lower bound, the estimate is not.
    */
    ...
    const selfConsumed = (keep: (date: string) => boolean, producedWh: number): number =>
      Math.max(measuredSelfConsumed(keep), (producedWh / 1000) * assumedShare);
    ...
    */
    ...
    const [todayP, month, year, lifetime] = windows.map(([wh, keep]) =>
      this.buildPeriod(
        ...
        // Estimated only when the assumption actually lifted the measurement.
        (wh / 1000) * assumedShare > measuredSelfConsumed(keep),
        retail,
        program,
