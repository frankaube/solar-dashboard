...
import { MeasuredShare, measuredSelfConsumptionShare } from './self-consumption-share';
import { RateEntry, rateOn, retailOf, weightedRate } from './rate-history';
...
export interface SavingsPeriod {
  ...
  */
   ...
   *
   * Alongside producedKwh this says how much of the period rests on a measurement rather
   * than an assumption — which the flag above cannot, being one bit for a period that is
   * routinely part one and part the other. A published usage export always lags, so "most
   * of this month is measured and the last few days are not" is the normal state, not an
   * edge case.
   */
...
export class SavingsService {
  ...
  private async mainsExportedKwhByDate(): Promise<Map<string, number> | null> {
    /*
      The utility's own meter first, where it has been imported.

      Both measure the same boundary, but only one of them is what the bill is calculated
      from — and it needs no hardware, reaches back to the day the panels went live, and
      cannot drift out of calibration. A clamp is the finer instrument; the meter is the
      authoritative one, and where they disagree the money follows the meter.

      Days the meter recorded no export while the array produced are absent from this map
      rather than present as zero, so they fall through to the estimate instead of
      reporting that everything stayed home.
    */
    const fromUtility = await this.utility.exportedKwhByDate();
    if (fromUtility.size > 0) return fromUtility;
   ...
   *
   * The arithmetic that used to live here was correct, but it was correct only for one
   * province — the tariff was the shape of the code rather than data it consumed.
  ...
  private buildPeriod(
    ...
    */
    buckets?: FlowBucket[],
    /** Production whose self-consumption came from a meter rather than an assumption. */
    measuredProducedKwh = 0,
  ): SavingsPeriod {
    ...
    return {
      ...
      measuredProducedKwh: round1(Math.min(measuredProducedKwh, producedKwh)),
  ...
  /**
   * What the meter says this house's self-consumption share actually is.
   *
   * Read by Settings so the owner can see the measured figure beside the one they typed
   * before deciding which to trust. Recomputed rather than lifted out of the savings
   * cache: it is asked for once, on a page nobody leaves open, and a stale percentage
   * next to a toggle offering to adopt it is worse than a slow one.
   */
  async selfConsumptionEstimate(): Promise<MeasuredShare & { enabled: boolean; configuredPct: number | null }> {
    const [config, mainsByDate, dailyProduction] = await Promise.all([
      this.readings.getConfig(),
      this.mainsExportedKwhByDate(),
      this.readings.getDailyEnergy(LIFETIME_DAYS),
    ]);
    const share = mainsByDate
      ? measuredSelfConsumptionShare(
          dailyProduction
            .filter((row) => row.energyWh > 0 && mainsByDate.has(row.date))
            .map((row) => ({
              date: row.date,
              producedKwh: row.energyWh / 1000,
              exportedKwh: mainsByDate.get(row.date) ?? 0,
            })),
        )
      : {
          pct: null,
          ...
          producedKwh: 0,
          selfConsumedKwh: 0,
          reason: 'No meter data yet — import a usage export, or fit a clamp on the mains.',
        };
    return {
      ...share,
      enabled: config.selfConsumptionAuto,
      configuredPct: config.selfConsumptionPct,
    };
  }
  ...
  async getSavings(): Promise<SavingsDto> {
    ...
    */
    ...
    const [evByDate, battByDate, mainsByDate, dailyProduction, rateEntries] = await Promise.all([
      ...
      this.readings.getDailyEnergy(LIFETIME_DAYS),
      /*
        Recorded rate changes, oldest first. Empty on every install that has never entered
        one, which is what keeps this change invisible until somebody uses it.
      */
      this.prisma.rateEntry.findMany({ orderBy: { effectiveFrom: 'asc' } }),
    ]);
    ...
    */
    /*
      The share applied to days no meter covered.

      Typed into Settings by default, because on most installs nothing can measure it. Where
      meter data exists the app can do better than a round number: what stayed home across
      the covered days is production minus export, and that share belongs to this roof and
      these habits rather than to a plausible guess. Still an assumption when applied to
      days nobody watched — but a calibrated one.

      Only when asked. Silently replacing a figure someone entered with one computed here
      would leave them looking at a number they did not choose and cannot trace, and the
      first thing they would do is type the old one back.
    */
    const measuredShare = mainsByDate
      ? measuredSelfConsumptionShare(
          dailyProduction
            .filter((row) => row.energyWh > 0 && mainsByDate.has(row.date))
            .map((row) => ({
      ...
      : null;
    const autoShare = config.selfConsumptionAuto && measuredShare?.pct !== null && measuredShare !== null
      ? measuredShare.pct
      : null;
    const sharePct = autoShare ?? config.selfConsumptionPct ?? 0;
    const assumedShare = sharePct ? sharePct / PERCENT : 0;
    ...
    */
    /**
     * Split a period into the days a meter can speak for and the days it cannot.
     *
     * The first version of this demanded that a meter cover the *whole* period or be
     * ignored, which was right about the danger and wrong about the remedy. A published
     * usage export always lags — the utility's own file ends weeks before today — so no
     * period is ever fully covered, and an all-or-nothing rule quietly threw away every
     * measurement the moment it was imported. Meanwhile the danger it guarded against is
     * real: subtracting a clamp's exports from a lifetime total reports every pre-clamp
     * kWh as used at home.
     *
     * Both are answered by measuring per day. Days with a reading are measured; days
     * without fall back to the estimate; the two are added. Nothing is discarded and
     * nothing is extrapolated onto a day nobody watched.
     */
    const splitByCoverage = (
      keep: (date: string) => boolean,
    ): { coveredKwh: number; exportedKwh: number; uncoveredKwh: number } => {
      let coveredKwh = 0;
      let exportedKwh = 0;
      let uncoveredKwh = 0;
      for (const row of dailyProduction) {
        if (!keep(row.date) || row.energyWh <= 0) continue;
        const exported = mainsByDate?.get(row.date);
        if (exported === undefined) uncoveredKwh += row.energyWh / 1000;
        else {
          coveredKwh += row.energyWh / 1000;
          exportedKwh += exported;
        }
      }
      return { coveredKwh, exportedKwh, uncoveredKwh };
    };

    /** EV and battery on days no meter covered — the floor for the estimated remainder. */
    const measuredOnUncovered = (keep: (date: string) => boolean): number => {
      const uncovered = (date: string): boolean => keep(date) && !mainsByDate?.has(date);
      return sum(evByDate, uncovered) + sum(battByDate, uncovered);
    };

    const selfConsumed = (keep: (date: string) => boolean, producedWh: number): number => {
      const { coveredKwh, exportedKwh, uncoveredKwh } = splitByCoverage(keep);
      if (mainsByDate === null || coveredKwh === 0) {
        return Math.max(measuredSelfConsumed(keep), (producedWh / 1000) * assumedShare);
      }
      const measured = selfConsumedFromMains(coveredKwh, exportedKwh);
      /*
        The remainder uses the period total rather than the summed daily figures, so a day
        the array produced but the rollup has not caught up on is still valued rather than
        silently dropped between the two.
      */
      const remainderKwh = Math.max(uncoveredKwh, producedWh / 1000 - coveredKwh);
      const estimatedPart = Math.max(measuredOnUncovered(keep), remainderKwh * assumedShare);
      return measured + Math.max(0, estimatedPart);
    };

    /** Production whose self-consumption came from a meter rather than an assumption. */
    const measuredProducedKwh = (keep: (date: string) => boolean): number =>
      mainsByDate === null ? 0 : splitByCoverage(keep).coveredKwh;

    /** True when any part of the period leaned on the owner's estimate. */
    const leanedOnEstimate = (keep: (date: string) => boolean, producedWh: number): boolean => {
      const { coveredKwh } = splitByCoverage(keep);
      const remainderKwh = producedWh / 1000 - coveredKwh;
      if (remainderKwh <= 0.01) return false;
      return remainderKwh * assumedShare > measuredOnUncovered(keep);
    };
    ...
    */
    ...
    /*
      Each period priced at the rate that was actually in effect across ITS days.

      One current price applied to all history means every figure changes retroactively the
      day a utility raises its rate — a savings total written down last winter stops
      matching the one on screen, and nothing says why. Weighted by production rather than
      by day count, because a rate in effect through December carried less energy than one
      in effect through June.

      Falls back to the single configured price when no rate history has been recorded,
      which is every existing install until somebody enters a change. Nobody's numbers move
      on upgrade.
    */
    const fallbackRate: RateEntry = {
      effectiveFrom: '1970-01-01',
      pricePerKwh: config.electricityPricePerKwh,
      hstRate,
      priceIncludesTax: config.priceIncludesTax !== false,
    };
    const rateFor = (keep: (date: string) => boolean) =>
      weightedRate(rateEntries, dailyProduction.filter((row) => keep(row.date)), fallbackRate);

    const [todayP, month, year, lifetime] = windows.map(([wh, keep]) =>
      this.buildPeriod(
        wh,
        selfConsumed(keep, wh),
        // Estimated when ANY part of the period leaned on the assumption. A period that is
        // three-quarters measured is still partly assumed, and a flag that only fires at
        // 100% assumed would call it a measurement.
        leanedOnEstimate(keep, wh),
        rateFor(keep).retail,
        program,
        hourly ? buildBuckets(hourly.production, hourly.selfConsumption, keep) : undefined,
        measuredProducedKwh(keep),
      ),
    );

    /*
      The rate in effect TODAY, for the figures that are about now.

      `marginal` answers "what is one more kilowatt-hour worth", and the periods above
      answer "what was it worth" — different questions, and only the second one is
      historical. Publishing the configured price here while pricing history from the
      recorded rates would put two different answers on the same page, with the one people
      act on being the stale one.
    */
    const currentRate = rateOn(rateEntries, today) ?? fallbackRate;
    const currentRetail = retailOf(currentRate);

    const value: SavingsDto = {
      rates: {
        retailPerKwh: currentRetail,
        hstRate: currentRate.hstRate,
        perKwh: programRates(program, currentRetail),
        marginal: marginalValue(program, currentRetail),
      },
      today: todayP,
      month,
      year,
