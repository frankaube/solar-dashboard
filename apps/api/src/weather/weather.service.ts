 ...
 */
const LATITUDE_SETTING_KEY = SITE_SETTING_KEYS.latitude;
const LONGITUDE_SETTING_KEY = SITE_SETTING_KEYS.longitude;
...
export class WeatherService implements OnModuleInit, OnModuleDestroy {
   ...
   *
   * The rules — settings over env, blanks as absent, and the refusal to accept Null Island —
   * are shared with the car's home, which is usually the same driveway. See
   * common/coordinates for why one of those cases needed naming.
   */
  async getLocation(): Promise<Coordinates | null> {
    const rows = await this.prisma.setting.findMany({
      where: { key: { in: [LATITUDE_SETTING_KEY, LONGITUDE_SETTING_KEY] } },
    });
    return resolveSiteLocation((key) => rows.find((row) => row.key === key)?.value);
  }

  async setLocation(latitude: number, longitude: number): Promise<void> {
    const { coordinates, problems } = parseCoordinates(latitude, longitude);
    if (!coordinates) throw new Error(problems[0]?.message ?? 'Invalid location');
    for (const [key, value] of [
      [LATITUDE_SETTING_KEY, coordinates.latitude],
      [LONGITUDE_SETTING_KEY, coordinates.longitude],
    ] as const) {
