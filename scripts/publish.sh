...
if [ -f "$DENY_FILE" ]; then
  ...
  # And again with every line joined, because git grep matches within a line.
  #
  # A two-word term slips through the moment an editor reflows a paragraph: the first word
  # ends one line, the second opens the next behind a comment leader, and a search for the
  # whole phrase finds nothing. One had been sitting in the published tree for exactly that
  # reason while this audit reported clean on every run.
  #
  # In node rather than shell: joining the tree with three processes per file took over ten
  # minutes on Windows, which is the same as not running.
  set +e
  node "$ROOT/scripts/wrapped-scan.mjs" "$DENY_FILE" "${EXCLUDE[@]}"
  wrapped_status=$?
  set -e
  [ "$wrapped_status" -eq 0 ] || FOUND=1
fi
