...
/*
  The Windows installer, when one has been built beside the tarballs.

  Optional because it cannot be built here: Inno Setup is a Windows compiler and this
  script runs on Linux in CI, so the installer arrives from a separate job that ran on
  windows-latest. Absent, the release is exactly what it was before.

  Listed in the same SHA256SUMS as everything else so it is covered by the one signature
  the maintainer makes. A second checksum file would be a second thing to sign, and the
  one that got forgotten would be the one nobody could verify.
*/
const installer = 'SolarDashboardSetup.exe';
const installerPath = join(OUT, installer);
if (existsSync(installerPath)) {
  const digest = createHash('sha256').update(readFileSync(installerPath)).digest('hex');
  sums.push(`${digest}  ${installer}`);
  console.log(
    `> including ${installer}  ${(statSync(installerPath).size / 1024 / 1024).toFixed(1)} MB  ${digest.slice(0, 16)}…`,
  );
}
