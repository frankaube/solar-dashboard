...
if [ "$MODE" = "check" ]; then
  # Deliberately touch_checked, not record.
  #
  # This branch is the normal state of a notify-only install: something is available and
  # nobody has asked for it. Writing a "refused" outcome here overwrote the record of a
  # genuine failure — and because the timer is check-only most of the time, that erased the
  # memory of a rolled-back version within the hour. The guard below then saw "refused"
  # instead of "rolled-back", did not fire, and reinstalled the broken build. Install,
  # fail, roll back, every hour, each cycle taking the service down.
  #
  # Found on real hardware: 0.1.2 was installed and rolled back twice in a row, with a
  # check-only run in between doing the erasing.
  log "not installing: nothing requested it, and unattended installs are off or outside the window"
  touch_checked
  exit 0
fi
...
# The service scripts too, including this one.
#
# systemd execs update.sh directly, so a bundle whose scripts arrived without the bit would
# leave the updater unable to run again — it would install itself into a state where it can
# never install anything else. Repairing it here means a machine carrying an older bundle
# fixes itself on the next update rather than needing a hand.
for script in "$INSTALL_DIR"/service/*.sh; do
  [ -f "$script" ] && chmod 755 "$script"
done

# Unit files, which a build can change and an update could not previously deliver.
#
# Everything above replaces what is under /opt. systemd reads from /etc/systemd/system, so
# until this existed a release could add a timer or change a unit and the machine would
# carry the new file around in /opt while running the old one indefinitely. The reliability
# work is the case that exposed it: a watchdog that is shipped but never enabled is worse
# than no watchdog, because the dashboard says it is there.
#
# The base units are overwritten deliberately — customisation goes in drop-ins under
# <unit>.d/, which this does not touch, and that is the mechanism install.sh already uses
# for the data-directory mount.
for unit in "$INSTALL_DIR"/service/*.service "$INSTALL_DIR"/service/*.timer; do
  [ -f "$unit" ] || continue
  name="$(basename "$unit")"
...
# Timers ship disabled; enabling here is what makes a new one start working on update
# rather than at the next fresh install, which on a long-lived box is never.
if [ -f /etc/systemd/system/solar-netwatch.timer ]; then
  systemctl enable --now solar-netwatch.timer >/dev/null 2>&1 || true
fi
# And the hardware watchdog, for machines installed before it was configured.
if [ -e /dev/watchdog ] && [ ! -f /etc/systemd/system.conf.d/solar-watchdog.conf ]; then
  mkdir -p /etc/systemd/system.conf.d
  printf '[Manager]
RuntimeWatchdogSec=15
RebootWatchdogSec=2min
'     >/etc/systemd/system.conf.d/solar-watchdog.conf
  log "enabled the hardware watchdog (takes effect at the next boot)"
fi
