...
# Nothing happened, so nothing is recorded as an outcome — only that we looked. These
# used to assert result=refused, which is what allowed a routine check to overwrite the
# record of a real failure and re-arm an install loop.
check "no outcome recorded" "$(state .result)" ""
check "but the check is timestamped" "$([ -n "$(state .checkedAt)" ] && echo yes || echo no)" "yes"
...
echo "== 19. a check-only run must not erase the memory of a failure =="
# The sequence that defeated the no-retry guard on real hardware: a version fails and is
# rolled back, an ordinary notify-only tick happens (which is what the hourly timer does
# almost every time), and then an unattended run reinstalls the build that just failed.
# Install, fail, roll back, every hour, each cycle taking the service down.
#
# Republishes 0.2.0 first: the previous case leaves the feed offering something else, and
# a request that does not match the feed is refused for an entirely different reason.
build_release 0.2.0 bbbbbbb
publish_feed 0.2.0
reset_install; conf; policy stable false; request 0.2.0
echo dead > /tmp/health-mode
sed -i 's/^HEALTH_TIMEOUT=60/HEALTH_TIMEOUT=3/' /opt/solar-dashboard/service/update.sh
run >/dev/null 2>&1
check "0.2.0 rolled back" "$(state .result)" "rolled-back"
check "recorded against 0.2.0" "$(state .toVersion)" "0.2.0"

echo normal > /tmp/health-mode
policy stable false                    # notify-only: the timer's usual tick
OUT=$(run)
has "reports without installing" "$OUT" "not installing"
check "failure record survives the check" "$(state .result)" "rolled-back"
check "and still names 0.2.0" "$(state .toVersion)" "0.2.0"

policy stable true "$(date +%-H)"      # now let it install unattended
OUT=$(run)
has "refuses to retry" "$OUT" "already failed"
check "still on 0.1.0" "$(jq -r .version /opt/solar-dashboard/version.json)" "0.1.0"
check "service was never stopped" "$([ -f /tmp/systemctl.log ] && grep -c 'systemctl stop' /tmp/systemctl.log || echo 0)" "2"
