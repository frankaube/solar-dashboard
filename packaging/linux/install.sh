...
# The network watchdog, under the same rule: it runs as root and can reboot the machine, so
# it must not be writable by the service user.
if [ -f "$HERE/netwatch.sh" ]; then
  cp "$HERE/netwatch.sh" "$INSTALL_DIR/service/netwatch.sh"
  chmod 755 "$INSTALL_DIR/service/netwatch.sh"
fi
...
if [ ! -f "$INSTALL_DIR/.env" ]; then
  {
    ...
    echo ""
    # Commented out, not blank. An empty environment variable is "" rather than unset, and
    # Number("") is 0 — so writing these as bare keys put every install at 0 degrees, 0
    # degrees and fetched a real forecast for the Gulf of Guinea. Set the site in Settings;
    # these exist for seeding a container that nobody will click through.
    echo "# Weather forecast. Leave commented — set the location in Settings > Hardware."
    echo "#SITE_LATITUDE="
    echo "#SITE_LONGITUDE="
    echo ""
...
if [ -f "$HERE/solar-netwatch.service" ]; then
  cp "$HERE/solar-netwatch.service" "$HERE/solar-netwatch.timer" /etc/systemd/system/
fi

# The hardware watchdog.
#
# The Pi has one and systemd does not use it by default, so a kernel that stops scheduling
# takes the machine off the network until somebody walks over and pulls the power. This is
# the layer under the network watchdog: that one repairs a link, this one covers the case
# where nothing is running to repair anything.
#
# A drop-in, not an edit to system.conf, so an OS upgrade does not quietly revert it and
# nothing here has to parse a file it does not own. Left alone if the machine has no
# watchdog device — a VM or a container should not be told to reboot itself.
if [ -e /dev/watchdog ]; then
  mkdir -p /etc/systemd/system.conf.d
  {
    echo "# Installed by Solar Dashboard. A hung kernel reboots itself rather than sitting"
    echo "# unreachable until somebody notices."
    echo "[Manager]"
    # 15s of a genuinely stopped kernel. Long enough that heavy IO cannot trip it.
    echo "RuntimeWatchdogSec=15"
    # And a stuck *shutdown* is its own way to never come back.
...
# Watches the default route and repairs it — see netwatch.sh for what it escalates through.
if [ -f /etc/systemd/system/solar-netwatch.timer ]; then
  systemctl enable --now solar-netwatch.timer
fi
